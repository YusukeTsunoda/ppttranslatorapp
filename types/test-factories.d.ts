...
// 各モデル用のファクトリーインターフェース
interface UserFactory extends BaseFactory<User> {
  /**
   * 管理者ユーザーを作成
   */
...
interface FileFactory extends BaseFactory<File> {
  /**
   * 特定のユーザーに属するファイルを作成
   */
...
interface TranslationHistoryFactory extends BaseFactory<TranslationHistory> {
  /**
   * 特定のユーザーに属する翻訳履歴を作成
   */
...
interface ActivityLogFactory extends BaseFactory<ActivityLog> {
  /**
