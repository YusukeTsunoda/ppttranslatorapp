...
export class FilePathManager {
  ...
  getTempPath(userId: string, fileId: string, type: 'original' | 'translated' = 'original'): string {
    ...
    return join(this.config.tempDir, userId, 'uploads', `${fileId}_${type === 'translated' ? 'translated' : 'original'}.pptx`);
  }
  ...
  async findActualFilePath(userId: string, fileId: string, type: 'original' | 'translated' = 'original'): Promise<string | null> {
    try {
      ...
      const file = files.find(f => 
        f.startsWith(fileId) && 
        (type === 'original' ? !f.includes('_translated') : f.includes('_translated'))
      );
